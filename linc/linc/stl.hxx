...
struct stl_facet {
  ...
  stl_facet
  rotated(const Eigen::Quaternion<float, Eigen::DontAlign> &rot) const {
    stl_facet out;
...
struct stl_file {
  ...
  size_t memsize() const {
    return sizeof(*this) + sizeof(stl_facet) * facet_start.size() +
           sizeof(stl_neighbors) * neighbors_start.size();
  }
...
struct indexed_triangle_set {
  ...
  size_t memsize() const {
    return sizeof(*this) +
           sizeof(stl_triangle_vertex_indices) * indices.size() +
           sizeof(stl_vertex) * vertices.size();
  }

  std::vector<stl_triangle_vertex_indices> indices;
  std::vector<stl_vertex> vertices;
  // FIXME add normals once we get rid of the stl_file from TriangleMesh
  // completely. std::vector<stl_normal>
  // normals
};
...
extern bool stl_write_binary(stl_file *stl, const char *file,
                             const char *label);
...
inline void stl_scale(stl_file *stl, float factor) {
...
template <typename T>
inline void stl_transform(stl_file *stl,
